...
url.yahoo.finance.sector <- paste(url.yahoo.finance.base, 's_conameu.csv', 
                                  sep='/')
url.yahoo.industry.list <- 'https://biz.yahoo.com/ic/ind_index_alpha.html'
...
yahoo.download.csv.as.df <- function(url, set.id=TRUE) {
  ...
  # html.names <- as.vector(xpathSApply(html, "//a", xmlValue))
  ...
  #html.names <- gsub("https://us.rd.yahoo.com/finance/industry/quote/colist/*http://biz.yahoo.com/p/a/([a-z])", "", html.names)
